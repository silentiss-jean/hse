...
const build_signature = "2026-03-20_refonte_store_phase8";
...
(function () {
  ...
  class hse_panel extends HTMLElement {
    ...
    // ── Helpers accès direct aux state bridges (phase 8) ──────────────────
    ...
    async _fetch_reference_status(for_entity_id) {
      ...
      if (this._reference_status_polling) return this._cg('reference_status');
    ...
    async _render_config() {
      ...
      const _effective_ref = () => this._cg('selected_reference_entity_id') || this._cg('current_reference_entity_id') || null;

      const _ensure_pricing_draft = () => {
        if (!this._cg('pricing_draft')) {
          ...
          this._deep_fill_missing(this._cg('pricing_draft'), this._cg('pricing_defaults') || {});
        }
      };

      const _cost_ids = () => {
        _ensure_pricing_draft();
        const arr = this._cg('pricing_draft')?.cost_entity_ids;
      ...
      const _remove_ref_from_cost = () => {
        ...
        this._cg('pricing_draft').cost_entity_ids = ids.filter((x) => x !== ref);
        return true;
      };
      ...
      const _update_from_catalogue = (cat) => {
        ...
        if (this._cg('selected_reference_entity_id') == null) {
          this._cs('selected_reference_entity_id', cur);
        }

        const snapshot = window.hse_config_view._reference_status_from_catalogue?.(
          cat,
          this._cg('selected_reference_entity_id') || cur || null
        );
        if (snapshot && typeof snapshot === "object") {
          this._cg('reference_statu')s = {
            ...(this._cg('reference_status') || {}),
            ...snapshot,
            entity_id: snapshot.entity_id || cur || this._cg('selected_reference_entity_id') || null,
          };
        }

        if (this._cg('pricing_draft') && _remove_ref_from_cost()) {
      ...
      const _candidate_index = () => {
        const items = Array.isArray(this._cg('scan_result')?.candidates) ? this._cg('scan_result').candidates : [];
        const map = new Map();
      ...
      if (!this._cg('catalogue') && !this._cg('loading')) {
        ...
        try {
          this._cg('scan_resul')t = await window.hse_scan_api.fetch_scan(this._hass, {
      ...
      window.hse_config_view.render_config(container, this._config_state, async (action, value) => {
        ...
        if (action === "cost_auto_select") {
          ...
          _ensure_pricing_draft();
          this._cg('pricing_draft').cost_entity_ids = entity_ids;
          if (_remove_ref_from_cost()) {
            this._cs('pricing_message', "Garde-fou: le capteur de r\u00e9f\u00e9rence a \u00e9t\u00e9 retir\u00e9 des capteurs de calcul.");
          } else {
        ...
        if (action === "pricing_list_replace") {
          ...
          this._cg('pricing_draft').cost_entity_ids = ids;
        ...
        if (action === "select_reference") {
          ...
          this._cs('message', null);
          this._cs('reference_status_error', null);
          const next_effective_entity_id = value || this._cg('current_reference_entity_id') || null;
          if ((this._cg('reference_status')?.entity_id || null) !== next_effective_entity_id) {
        ...
        if (action === "pricing_patch") {
          ...
          _ensure_pricing_draft();
          _deep_set(this._cg('pricing_draft'), path, v);
        ...
        if (action === "pricing_save") {
          ...
          this._render();
          try {
            await window.hse_config_api.set_pricing(this._hass, this._cg('pricing_draft'));
            const pricingResp = await window.hse_config_api.fetch_pricing(this._hass);
            this._cs('pricing_draft', null);
            _update_from_pricing(pricingResp);
        ...
        if (action === "save_reference") {
          const entity_id = this._cg('selected_reference_entity_id');
          if (!entity_id) {
          ...
          if (ids.includes(entity_id)) {
            this._cg('pricing_draft').cost_entity_ids = ids.filter((x) => x !== entity_id);
    ...
    async _render_diagnostic() {
      ...
      if (!this._dg('data') && !this._dg('loading')) {
        this._ds('loading', true);
        try {
          this._ds('data', await _wrap_last("fetch_catalogue", () => diag_api.fetch_catalogue(), { method: "get", path: "hse/unified/catalogue", body: null }));
          this._ds('error', null);
        } catch (err) {
          this._ds('error', this._err_msg(err));
        } finally {
          this._ds('loading', false);
        }
      }

      if (this._dg('error')) {
        container.appendChild(el("div", "hse_card", `Erreur: ${this._dg('error')}`));
        return;
      }
      ...
      const _filtered_ids = () => {
        ...
        if (!fn) return [];
        return fn(this._dg('data'), this._dg('filter_q')).map((x) => x.id);
      };

      const _filtered_entity_ids = () => {
        const filtered = window.hse_diag_view?._filtered_escalated_items?.(this._dg('data'), this._dg('filter_q')) || [];
      ...
      const _all_entity_ids = () => {
        const items = this._dg('data')?.items || {};
        return Array.from(new Set(Object.values(items).map((x) => x?.source?.entity_id).filter(Boolean))).sort();
      };

      window.hse_diag_view.render_diagnostic(container, this._dg('data'), this._diag_state, async (action, payload) => {
        if (action === "toggle_advanced") {
          this._ds('advanced', !this._dg('advanced'));
          this._render();
          return;
        }
        if (action === "filter") {
        ...
        if (action === "check_coherence") {
          ...
          this._render();
          try {
            this._ds('check_result', await _wrap_last("diagnostic_check", () => diag_api.check_consistency(req), { method: "post", path: "hse/unified/diagnostic/check", body: req }));
            this._ds('check_error', null);
          } catch (err) {
    ...
    _render_scan() {
      ...
      window.hse_scan_view.render_scan(container, this._scan_result, this._scan_state, async (action, value) => {
        ...
        if (action === "open_all") { this._scan_state.open_all = true; this._storage_set("hse_scan_open_all", "1"); this._render(); return; }
        if (action === "close_all") {
        ...
        if (action === "scan") {
          ...
          this._render();
          try {
            this._scan_result = await window.hse_scan_api.fetch_scan(this._hass, { include_disabled: false, exclude_hse: true });
          } catch (err) {
...
})();      // ── _diag_state helpers (phase 8) ──────────────────────────────────────
      // _dg(k)    → window.hse_diag_state?.get(k)
      // _ds(k, v) → window.hse_diag_state?.set(k, v)point - hse_panel.js */
const build_signature = "2026-03-20_refonte_store_phase8";
...
(function () {
  ...
  customElements.define("hse-panel", hse_panel);
})();      // ── _config_state helpers (phase 8) ────────────────────────────────────
      // _cg(k)    → window.hse_config_state?.get(k)
      // _cs(k, v) → window.hse_config_state?.set(k, v)point - hse_panel.js */
const build_signature = "2026-03-20_refonte_store_phase8";
